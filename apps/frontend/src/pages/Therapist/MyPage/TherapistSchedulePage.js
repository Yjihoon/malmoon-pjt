...
function TherapistSchedulePage() {
    ...
    const fetchMonthlySchedules = async (date) => {
      ...
      try {
        const response = await api.get(
          `/schedule?therapistId=${user.memberId}&startDate=${startDate}&endDate=${endDate}`,
          {
            headers: { Authorization: `Bearer ${user.accessToken}` },
          }
        );
        const scheduledDates = new Set(
          response.data.map((schedule) => formatDate(new Date(schedule.date)))
        );
  ...
  useEffect(() => {
    const fetchSchedulesForDate = async () => {
      ...
      try {
        ...
        const response = await api.get(
          `/schedule/therapist/date?date=${dateString}`,
          {
            headers: { Authorization: `Bearer ${user.accessToken}` },
          }
        );
      ...
      const dummyIndividualTools = [
        {
          ...
          type: "AAC",
          name: "그림카드 세트 A",
          description: "다양한 사물, 동물 그림 카드 (50장)",
          category: "어휘",
          lastModified: "2025-07-20",
          imageUrl: "https://via.placeholder.com/100x100?text=AAC_Card1",
        },
        {
          id: "aac2",
          type: "AAC",
          name: "문장 구성 보드",
          description: "주어-동사-목적어 연습 보드",
          category: "문법",
          lastModified: "2025-07-18",
          imageUrl: "https://via.placeholder.com/100x100?text=AAC_Board",
        },
        {
          id: "filter1",
          type: "Filter",
          name: "강아지 귀 필터",
          description: "화상 캠에 강아지 귀를 추가합니다.",
          category: "동물",
          lastModified: "2025-07-22",
          imageUrl: "https://via.placeholder.com/100x100?text=Filter_Dog",
        },
      ];
      ...
      const dummyToolSets = [
        {
          id: "set1",
          name: "초기 언어 발달 세션용",
          description: "그림카드와 문장 보드를 활용한 기초 세션",
          lastModified: "2025-07-25",
          toolIds: ["aac1", "aac2", "filter1"],
        },
      ];
  ...
  useEffect(() => {
    ...
    const fetchTitles = async () => {
      try {
        const response = await api.get(
          `/storybooks/titles?classification=${selectedClassification}`,
          {
            headers: { Authorization: `Bearer ${user.accessToken}` },
          }
        );
  ...
  useEffect(() => {
    ...
    const fetchPageRange = async () => {
      try {
        const response = await api.get(
          `/storybooks/pages?title=${encodeURIComponent(selectedTitle)}`,
          {
            headers: { Authorization: `Bearer ${user.accessToken}` },
          }
        );
        console.log(
          "DEBUG: /storybooks/pages API response data:",
          response.data
        );
        const { minPage, maxPage, storybookId } = response.data;
        setSelectedFairyTaleInfo({
          title: selectedTitle,
          minPage,
          maxPage,
          storybookId,
      ...
      if (monthlyScheduledDates.has(dateString)) {
        return (
          <div className="dot-marker-container">
            <span className="dot-marker schedule-marker"></span>
          </div>
        );
      }
    }
  ...
  const isSessionReady = (scheduleTime) => {
    ...
    const diffMinutes =
      (scheduleDateTime.getTime() - now.getTime()) / (1000 * 60);
  ...
  const handleCreateSessionRoom = () => {
    ...
    console.log(
      "DEBUG: currentSessionSchedule.memberId:",
      currentSessionSchedule.memberId
    );
    console.log("DEBUG: selectedFairyTaleInfo:", selectedFairyTaleInfo);
    console.log(
      "DEBUG: selectedFairyTaleInfo.storybookId:",
      selectedFairyTaleInfo?.storybookId
    );
    // --- 여기까지 추가 ---
    ...
    alert(
      `'${currentSessionSchedule.name}'님과의 수업방이 생성되었습니다!\n방 ID: ${roomId}`
    );

    let navPath = `/session/${roomId}?clientId=${
      currentSessionSchedule.memberId
    }&tools=${selectedToolsForSession.join(",")}`;
    if (selectedFairyTaleInfo) {
      navPath += `&fairyTaleTitle=${encodeURIComponent(
        selectedFairyTaleInfo.title
      )}&fairyTaleClassification=${selectedClassification}&startPage=${fairyTaleStartPage}&endPage=${fairyTaleEndPage}&storybookId=${
        selectedFairyTaleInfo.storybookId
      }`;
    }
    navigate(navPath);
  ...
  const totalFairyTalePages = Math.ceil(
    fairyTaleTitles.length / fairyTalesPerPage
  );
  ...
  const filterTools = allIndividualTools.filter(
  ...
  return (
    <Container fluid className="therapist-schedule-page-container">
      ...
      <Modal
        show={showToolSelectionModal}
        onHide={() => setShowToolSelectionModal(false)}
        centered
        size="lg"
      >
        <Modal.Header closeButton>
          <Modal.Title>
            {currentSessionSchedule
              ? `'${currentSessionSchedule.memberName}'님과의 수업 도구 선택`
              : "수업에 사용할 도구 선택"}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
              ...
              <div className="p-3">
                <Form.Group className="mb-3">
                  ...
                  >
                    ...
                    {fairyTaleClassifications.map((c) => (
                      ...
                      {totalFairyTalePages > 1 && (
                        <div className="d-flex justify-content-center align-items-center">
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              handleFairyTalePageChange(
                                fairyTaleCurrentPage - 1
                              )
                            }
                ...
                {selectedFairyTaleInfo && (
                  <Card className="mt-3 p-3 shadow-sm">
                    ...
                    <Card.Text>
                      총 페이지: {selectedFairyTaleInfo.maxPage} 페이지
                    </Card.Text>
                    <Row>
                      <Col>
                        <Form.Group className="mb-3">
                          ...
                          <Form.Control
                            ...
                            onChange={(e) =>
                              setFairyTaleStartPage(
                                Math.max(
                                  selectedFairyTaleInfo.minPage,
                                  Math.min(
                                    parseInt(e.target.value),
                                    selectedFairyTaleInfo.maxPage
                                  )
                                )
                              )
                            }
          ...
          <div className="mt-3 p-3 border-top">
            <p>
              현재 수업에 선택된 도구 ({selectedToolsForSession.length}개) 및
              동화:
            </p>
              ...
              !selectedFairyTaleInfo ? (
                ...
                <>
                  {selectedToolsForSession.map((toolId) => {
                    const tool = allIndividualTools.find(
                      (t) => t.id === toolId
                    );
                    return tool ? (
                      <Badge
                        key={toolId}
                        bg={tool.type === "AAC" ? "primary" : "success"}
                        className="me-2 mb-2"
                      >
                        {tool.name}
                        <Button
                          ...
                          onClick={() => handleToolSelectionInModal(toolId)}
                        >
                          &times;
                        </Button>
                      </Badge>
                  ...
                  {selectedFairyTaleInfo && (
                    <Badge bg="info" className="me-2 mb-2">
                      {selectedFairyTaleInfo.title} ({fairyTaleStartPage}p ~{" "}
                      {fairyTaleEndPage}p)
                      <Button
                        variant="link"
                        className="p-0 ms-2 text-decoration-none"
        ...
        <Modal.Footer>
          ...
          <Button
            variant="primary"
            onClick={handleCreateSessionRoom}
            disabled={
