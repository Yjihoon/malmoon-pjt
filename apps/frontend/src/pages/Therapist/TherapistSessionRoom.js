...
const CAMERA_KIT_API_TOKEN =
  "eyJhbGciOiJIUzI1NiIsImtpZCI6IkNhbnZhc2MyU2hNQUNQcm9kIiwidHlwIjoiSldUIn0.eyJhdWQiOiJjYW52YXMtY2FudmFzYXBpIiwiaXNzIjoiY2FudmFzLXMyc3Rva2VuIiwibmJmIjoxNzU0MDQ4MTI2LCJzdWIiOiJlODY4YTg3Ny1jYjVkLTQyMWEtOGE5Zi02MzlkZjExMDAyNTJ-U1RBR0lOR35hZGM0OWFjMy02NTU5LTRmNTctOWQ4Ny0yNTRjYzkwZjNhYzAifQ.EqNFYVSRYv7iEBCTBM-bxGvDEYOYernbf3ozbEhzB6g";
...
          const aacSetContentResponse = await api.get(
            `/aacs/sets/my/${toolBundle.aacSetId}`,
            {
              headers: { Authorization: `Bearer ${user.accessToken}` },
            }
          );
...
        });
        const allFilters =
          filterResponse.data.filters.map((item) => ({
            ...item,
...
        const selectedToolIds = toolsParam
          ? toolsParam.split(",").map((id) => id.trim())
          : [];
...
          const bundle =
            toolBundle && String(toolBundle.toolBundleId) === toolId
              ? toolBundle
              : null;
          if (bundle) {
            if (bundle.aacSetId) {
              const aacSetContentResponse = await api.get(
                `/aacs/sets/my/${bundle.aacSetId}`,
                {
                  headers: { Authorization: `Bearer ${user.accessToken}` },
                }
              );
...
                ...new Set([
                  ...initialAacIds,
                  ...aacsInSet.map((item) => String(item.id)),
                ]),
              ];
            }

            if (bundle.filterSetId) {
              const filterSetContentResponse = await api.get(
                `/filters/sets/my/${bundle.filterSetId}`,
                {
                  headers: { Authorization: `Bearer ${user.accessToken}` },
                }
              );
...
                ...new Set([
                  ...initialFilterIds,
                  ...filtersInSet.map((item) => String(item.filterId)),
                ]),
              ];
            }
          } else {
...
            } else if (
              allFilters.some((filter) => String(filter.filterId) === toolId)
            ) {
...
    }
  };
  const handleSendAacToLiveKit = useCallback(
    async (aacs) => {
...
        outputCKCanvasRef.current &&
        containerRef.current.contains(outputCKCanvasRef.current)
      ) {
...
      outputCanvasRef.current.style.visibility = "hidden";
    if (outputCKCanvasRef.current)
      outputCKCanvasRef.current.style.visibility = "visible";
...
        lensId,
        "1d348e19-0526-44d6-b5fb-cdc9f664b6bc"
      );
...
  const applyBackgroundFilter = useCallback(
    (imageUrl) => {
      if (cameraKitSessionRef.current) {
        cameraKitSessionRef.current.pause();
      }
...
  const { chatInput, setChatInput, sendChatMessage } = useChatLogic(
    roomRef,
    user,
    chatRoomId,
    setChatMessages
...
  // useFairyTaleLogic 훅에서 받아온 clearSentence 함수를 사용하여 새로운 함수를 선언합니다.
  // 이 함수를 SessionRoomContent 컴포넌트로 전달합니다.
  const handleClearSentence = useCallback(() => {
    setSelectedSentence(null); // 선생님 화면에서 선택된 문장 초기화
    clearSentence(); // 아동에게 지우기 명령 전송
  }, [clearSentence]);
...
    const unpublishAndHide = async (
      trackNamesToUnpublish,
      visibleVideo,
      hiddenCanvas1,
      hiddenCanvas2
    ) => {
...
      roomRef.current.localParticipant.videoTrackPublications.forEach(
        (publication) => {
          if (
            publication.track &&
            trackNamesToUnpublish.includes(publication.track.name)
          ) {
...
        try {
          const backgroundImage = await loadImageWithFetch(
            selectedBackgroundImageRef.current
          );
...
          const canvasVideoTrack = new LocalVideoTrack(
...
          if (outputCanvasRef.current)
            outputCanvasRef.current.style.visibility = "visible";
        if (outputCKCanvasRef.current)
          outputCKCanvasRef.current.style.visibility = "hidden";
        } catch (error) {
          console.error(
            "배경 이미지 로드에 실패하여 필터를 중단합니다.",
            error
          );
          setIsFilterActive(false);
          setSelectedBackgroundImage(null);
...
          );
          const unpublishPromises = publications.map((pub) =>
            roomRef.current.localParticipant
              .unpublishTrack(pub.track, true)
              .catch((e) => console.error("Failed to unpublish track", e))
          );
          await Promise.all(unpublishPromises);
...
          name: filter.name,
          url:
            filter.imageUrl && !filter.imageUrl.startsWith("http")
              ? `${filter.imageUrl}`
              : filter.imageUrl,
        }))
      );
    };
    if (rtcStatus === "connected" && allFilters.length > 0)
      fetchBackgroundImages();
  }, [rtcStatus, allFilters]);

  const toggleToolPanel = (toolType) => {
    setShowToolPanel((prev) =>
      prev && activeToolTab === toolType ? false : true
    );
...
      resolvedFilterIds.includes(String(filter.filterId))
    );
...
        endSession={() =>
          endSession(fairyTaleInfo?.title, currentFairyTalePage)
        }
